from layers import LSTM, TimeMean, Dense, MSELoss, SoftmaxCrossEntropyLoss
...
class LSTMNet:
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        Wx = 0.1*np.random.randn(D, 4*H)
        Wh = 0.1*np.random.randn(H, 4*H)
        b = np.zeros(4*H)
        Wl = 0.1*np.random.randn(H, O)
        bl = np.zeros(O)
...
if __name__ == '__main__':
    ...
    trainer.fit(x_train, y_train, x_test, y_test, epochs=50, batch_size=128, decay=True)
    trainer.plot_losses()
